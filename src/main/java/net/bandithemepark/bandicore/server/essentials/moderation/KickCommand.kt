...
class KickCommand: CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (!command.name.equals("kick", true)) return false
        ...
        if(args.size < 2) {
            sender.sendTranslatedMessage("kick-invalid-args", BandiColors.RED.toString())
            return true
        }
        ...
        messageJson.addProperty("action", "kick")
        ...
        sender.sendTranslatedMessage("kick-success", BandiColors.GREEN.toString(), MessageReplacement("player", target.name))
