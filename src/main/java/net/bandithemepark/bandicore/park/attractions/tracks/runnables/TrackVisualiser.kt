        ...
        if (delay >= 9) {
            ...
            for(session in BandiCore.instance.trackManager.editor.activeSessions) {
                ...
                for(node in session.layout.nodes) {
                    ...
                    session.player.spawnParticle(Particle.DUST, nodeLocation, 1, nodeOptions)
                    ...
                    for(curvePoint in node.curve) {
                        ...
                        if(curvePause == 3) {
                            ...
                            session.player.spawnParticle(Particle.DUST, curveLocation, 1, pathOptions)
                        }
                    }
                }
                ...
                for(rollNode in session.layout.rollNodes) {
                    ...
                    node.add(origin)
                    session.player.spawnParticle(Particle.DUST, node.toLocation(session.layout.world), 1, rollOptions)
                }

                for(segmentSeparator in session.layout.segmentSeparators) {
                    val node = segmentSeparator.position.getPathPoint().asVector()
                    node.add(origin)
                    session.player.spawnParticle(Particle.DUST, node.toLocation(session.layout.world), 1, segmentOptions)
                }

                for(trigger in session.layout.triggers) {
                    val node = trigger.position.getPathPoint().asVector()
                    node.add(origin)
                    session.player.spawnParticle(Particle.DUST, node.toLocation(session.layout.world), 1, triggerOptions)
                }
            }
        }
    }
