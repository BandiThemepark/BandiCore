...
class LeggingsCosmetic: CosmeticType("leggings") {
    var material: Material = Material.LEATHER_LEGGINGS
    ...
    override fun onEquip(player: Player, color: Color?, cosmetic: Cosmetic) {
        player.equipment.leggings = if(color != null) {
            ItemFactory(Material.LEATHER_LEGGINGS).setCustomModelData(customModelData).setDisplayName(cosmetic.getItemName()).setLore(cosmetic.getItemStackDescription()).setArmorColor(color).build()
        } else {
