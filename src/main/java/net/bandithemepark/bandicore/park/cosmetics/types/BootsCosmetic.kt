...
class BootsCosmetic: CosmeticType("boots") {
    var material: Material = Material.LEATHER_BOOTS
    ...
    override fun onEquip(player: Player, color: Color?, cosmetic: Cosmetic) {
        player.equipment.boots = if(color != null) {
            ItemFactory(Material.LEATHER_BOOTS).setCustomModelData(customModelData).setDisplayName(cosmetic.getItemName()).setLore(cosmetic.getItemStackDescription()).setArmorColor(color).build()
        } else {
